...
export async function POST(req: Request) {
  try {
    ...
    const [job, candidate] = await Promise.all([
      runAiStage("job_extraction", { userId: user.id }, () =>
        extractJob({
          job_description,
          title: body.title,
      ...
      runAiStage("candidate_extraction", { userId: user.id }, () =>
        extractCandidate({
          resume_text,
    ...
    const match = await runAiStage("role_match", { userId: user.id }, () =>
      matchRoleCandidate(job, candidate)
    );
