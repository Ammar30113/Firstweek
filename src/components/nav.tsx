...
export function MobileNav({ loggedIn, email }: { loggedIn: boolean; email: string | null }) {
  ...
  // The drawer is portalled to <body> so it escapes the nav's backdrop-filter
  // containing block — otherwise `position: fixed` is trapped inside the top bar.
  const drawer = (
    <div className="fixed inset-0 z-[60] flex justify-end">
      <button aria-label="Close menu" onClick={close} className="absolute inset-0 bg-black/50" />
      <div className="relative flex h-full w-[82%] max-w-xs flex-col bg-white shadow-2xl">
